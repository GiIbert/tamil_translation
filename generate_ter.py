...
def compute_TER(hyp_lines, ref_lines):
    ...
    ed_wrapper = ctypes.CDLL('./CharacTER/libED.so')
...
def load_result(dataset, testset):
    with open(f'datasets/{testset}/test.ta') as f:
        ref_lines = f.read().splitlines()
    with open(f'generation_results/{dataset}on{testset}.txt') as f:
...
def load_translate(dataset, testset):
    ...
    bpe='sentencepiece', sentencepiece_model='./bpe_model/ta.wiki.bpe.vs50000.model'
    )
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', help='Reference file', default='PMINDIA', required=True)
    parser.add_argument('-t', '--testset', help='Hypothesis file', default='PMINDIA', required=True)
...
if __name__ == '__main__':
    args = parse_args()
    dataset, testset = args.dataset, args.testset
    hyp_lines, ref_lines = load_result(dataset, testset)
    print('Start compute CharacTER')
    score = compute_TER(hyp_lines, ref_lines)
